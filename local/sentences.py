...
def split_text_into_sentences(text):
    pattern = r'(?<=[.!?…]) +'  # Regular expression pattern to split text into sentences
...
def main():
    # Load the data
    print("Loading data...")
    df = pd.read_json('data.json.1', lines=True, nrows=5000)
    ...
    sent_df.to_csv('sentences.csv', index=False)
    ...
    sent_df2 = sentence_split(df)
    sent_df2.to_csv('sentences2.csv', index=False)
